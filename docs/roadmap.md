...
- P8-1..P8-4 Audit keempat + **plugin VST3 sungguhan** — **SELESAI (30 Jul 2026, v0.23.0).**

  **Temuan audit yang diperbaiki.** (1) Looper dibangun tanpa syarat di setiap rantai, mengalokasi buffer rekam 60 detik di `prepare` — 23 MB di 48 kHz, 46 MB di 96 kHz — padahal di plugin ia tidak punya satu pun kontrol; `ChainOptions{withMetronome,withLooper}` sekarang membuatnya opsional, dan tanpa metronom `global.bpm` jatuh ke delay sebagai pemiliknya sehingga tetap ada satu tempo. (2) Latensi yang dilaporkan plugin hanya oversampling overdrive; latensi resampler NAM tidak dihitung dan `setLatencySamples` hanya dipanggil di `prepareToPlay`, jadi memuat model membuat track melenceng terhadap semua track lain — kini dijumlahkan dan diperiksa ulang di timer. Timer yang sama juga memanen model NAM yang dipensiunkan, yang sebelumnya tidak ada yang melakukannya di plugin (bocor satu model setiap ganti). (3) `parameterChanged` melakukan scan linear membandingkan `juce::String` per event otomasi, dari thread mana pun termasuk audio — diganti polling atomik per blok lewat pointer descriptor yang di-resolve sekali.

  **CI akhirnya meng-compile C++.** Sebelumnya `ci.yml` hanya frontend di ubuntu; untuk basis kode yang hidup di satu mesin, itu celah terbesar di seluruh proyek. Job `windows-latest` kini build engine + plugin + test tanpa SDK ASIO (sekaligus membuktikan jalur fallback WASAPI masih ter-compile), menjalankan suite native, lalu **`pluginval --strictness-level 10`**.

  **VST-A — plugin jadi benar-benar berguna.** `registerChainParameters` dipanggil tanpa `ChainExtras`, jadi `cabinet.irFile`/`irFileB`/`irBlend`, `reverb.irFile`, dan `nam.namFile` **tidak pernah terdaftar sama sekali**: di DAW cabinet-nya analitik dan NAM-nya passthrough, tanpa cara menjangkau file mana pun. Sekarang plugin punya `IrLibrary`/`NamLibrary` sendiri yang menunjuk folder `Documents\MilodikFX` yang sama (`UserPaths`, supaya app dan plugin tidak bisa berbeda). Ditambah: tempo mengikuti `getPlayHead()`, `getBypassParameter()` sehingga bypass host memakai crossfade 10 ms engine, parameter teks ikut tersimpan di state, dan preset `.json` yang sama dibaca plugin.

  **VST-B — UI yang sama di dalam DAW.** Kuncinya: `RestApiDispatcher` dan `HttpHandler` tidak pernah terikat socket — keduanya menerima `(method, path, query, body)` dan mengembalikan `(status, body)`. Jadi `PluginBackend` memakai ulang **handler yang identik** lewat transport berbeda, dan `PluginEditor` menyajikan **bundle UI yang sama persis** dari dalam biner lewat `WebBrowserComponent::withResourceProvider` JUCE 8, dengan `withNativeFunction` untuk tulisan. **Plugin tidak boleh membuka port** — beberapa instance dalam satu proyek akan berebut. Resource provider hanya menerima URL (tanpa method/body), justru itulah sebabnya tulisan lewat native function. Sisi frontend: `services/transport.ts` memilih sekali antara `fetch` dan jembatan `__juce__invoke`/`__juce__complete`; protokolnya ditulis sendiri, bukan mengimpor `juce.js` yang disajikan runtime — itu akan membuat bundle tidak lagi bisa ditanam sebagai satu berkas mandiri. SSE tidak ada di plugin, jadi `subscribeLevels` mengambil jalur polling yang memang sudah jadi fallback-nya. `isPluginHost()` menyembunyikan panel device, MIDI, update, dan looper — semua yang dimiliki host.

  **P7-4 tuntas: knob tersemat.** `PinnedControls` — sampai delapan kontrol per preset, muncul besar di Perform view. Menyimpan **referensi saja** (efek + parameter), tidak pernah nilai: aturan yang sama dengan scene, karena memanggil sesuatu tidak boleh melompatkan kontrol yang tidak sedang disentuh. Batas delapan disengaja (dua baris terbaca dari jarak panggung); daftar tanpa batas akan mengubah Perform kembali menjadi rack yang justru ingin dihindarinya. Preset schema 5; pin divalidasi terhadap registry saat dimuat, jadi preset dari build lain tidak menaruh knob kosong di layar panggung.

  **Sisanya.** State looper pindah ke payload meter yang sudah mengalir ~22 Hz (dulu polling 250 ms = 4 socket/detik di server thread-per-connection); **10 preset gitar & bass ikut tertanam di exe** dan ditulis saat pertama jalan tanpa pernah menimpa file yang sudah ada; sampah era Electron dibuang (`test-native-binding.js`, `troubleshoot.js`, laporan `SPRINT_*`/`TEST_*` yang lolos, skill Electron, `spike/nam-bench`); warning C4100 di `HttpHandler.h` dan variabel mati di `AudioDeviceController` dibersihkan.

  **Diuji:** 1.816.333 assertion backend, 203 test frontend, type-check, lint, E2E 52/52 (dua run bersih berturut-turut), dan **pluginval strictness 10 lolos tanpa satu temuan pun**.

  **Batas yang diakui jujur:** instance yang di-bypass tidak dikompensasi latensinya sesuai angka yang dilaporkan. Plugin belum pernah dijalankan di DAW sungguhan di sini — pluginval memvalidasi kontraknya, bukan selera host tertentu.

- P8-5 Modifier + pedal ekspresi di plugin — **SELESAI (30 Jul 2026).** `ModulationEngine` tidak ada sama sekali di plugin, jadi tremolo/auto-wah/wah mati di DAW dan panel Modifier di UI plugin (yang tidak pernah disembunyikan) gagal memuat — keputusan cakupan yang tidak pernah dinyatakan, dan itu lebih buruk daripada memilih salah satunya. Engine kini jalan di tempat yang sama seperti di aplikasi: di thread audio, tepat sebelum chain membaca parameter. **MIDI input dinyalakan untuk satu alasan**: wah adalah frekuensi contour yang disapu pedal, dan di host pedal itu datang sebagai controller message, bukan dari device yang dibuka plugin sendiri. `readControllers()` hanya menyimpan nilainya ke atomic — **tanpa dispatch pemetaan**, karena memanggil scene harus di-post ke message thread dan mem-post itu mengalokasi; persis alasan aplikasi mengerjakannya di thread MIDI JUCE. Jadi footswitch → scene/channel di plugin **masih belum ada**, disebut terang-terangan. Tiga interaksi yang masing-masing jadi bug kalau dibalik: otomasi host pada parameter termodulasi ditawarkan ke `setBase` dulu (menulis atomic langsung akan kalah dari sapuan setiap blok); `syncHostParametersFromChain` melaporkan **titik tengah**, bukan sampel sapuan; state menyimpan `baseOffset`. Sekalian mendaftarkan `/api/tuner`, yang plumbing-nya sudah dipasang tapi handler-nya terlewat. Diuji: pluginval strictness 10 bersih, 1.816.333 assertion backend, dan jendela plugin terbukti merender panel Preset (11 preset), Amp Model (NAM), dan Modifier lewat jembatan native.

- P8-6 Installer memasang VST3 + prosedur instalasi — **SELESAI (31 Jul 2026, v0.24.0).** Memasang plugin sebelumnya berarti membaca README, mencari path folder, membuatnya, lalu menempelkan bundle. Sekarang satu centang di installer (nyala secara default), mendarat di `{autocf}\VST3` — lokasi common-files per-pengguna pada instalasi biasa, lokasi sistem bila dijalankan elevated. **Setiap host VST3 memindai keduanya**, jadi tidak ada path DAW yang perlu diatur. Mencopot ikut membersihkannya; preset di Documents tetap tidak disentuh. Diverifikasi dengan benar-benar menjalankannya: pasang senyap exit 0 dengan aplikasi di tempatnya dan uninstaller yang berfungsi, copot senyap exit 0 dan bersih; payload VST3 dipastikan lewat log kompresi, bukan disimpulkan dari ukuran berkas.

  **Temuan saat mengujinya, dan sekarang ada di README:** host yang sedang memuat plugin **mengunci berkasnya**, dan installer tidak bisa menimpanya — tutup DAW dulu. Berlaku untuk semua plugin, tapi tidak ada yang senang menemukannya lewat pesan "access denied".

  Sekalian memperbaiki **flake `TunerDisplay`** yang memerahkan CI pada commit yang tidak menyentuh apa pun selain tiga berkas dokumentasi — bukti kuat bahwa itu flake lama, bukan regresi. Bacaan tuner datang lewat callback langganan, jadi jarumnya tertinggal satu render dari klik; di mesin ini mendarat di tick yang sama dan lolos, di runner CI yang terbebani tidak. Assertion-nya kini menunggu, bukan mengandaikan.
